        ...
        XposedBridge.hookAllConstructors(classActivityRecord,
            new XC_MethodHook(XCallback.PRIORITY_HIGHEST) {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    String packageName = Util.getFailsafeStringFromObject(param.thisObject, "packageName");
                    if(packageName==null)
                        return;
...
//						removePackage(packageName);
                    ...
                    /* DEPRECATED
                    if(mPackagesTasksList.containsKey(packageName)
                    ...
                    if (MainXposed.mCompatibility.ActivityRecord_ActivityStack != -1)
                        ...
                        mStackSupervisor =
                            param.args[MainXposed.mCompatibility.ActivityRecord_StackSupervisor];
                        mActivityStack =
                            MainXposed.mCompatibility.getActivityRecord_ActivityStack(
                                    mStackSupervisor
                            );
                    } else if (
                            Util.getFailsafeObjectFromObject(
                                    mStackSupervisor,
                                    param.thisObject,
                                    "mStackSupervisor"
                            )
                    ...
                    */
                    ...
                    // DEPRECATED isMovable = isMovable || isPackageMovable(packageName);
        ...
        XposedBridge.hookAllConstructors(classTaskRecord,
            new XC_MethodHook(XCallback.PRIORITY_HIGHEST) {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    String packageName = Util.getFailsafeStringFromObject(param.thisObject, "affinity");
                    if(packageName == null)
                        return;
//					isMovable = false;
...
//		}
    ...
    private static boolean checkBlackWhiteList(boolean flag, String packageName){
        ...
        switch (MainXposed.getBlackWhiteListOption()) {
            case 1: /* Always open apps in ext except blacklisted apps */
                flag=!MainXposed.isBlacklisted(packageName);
                break;
            case 2: /* Never open apps in ext + force whitelisted apps in ext */
                flag = MainXposed.isWhitelisted(packageName);
                break;
            case 3: /* Blacklist all apps & only allow whitelisted apps to be opened in ext */
...
//					isMovable = isMovable || (mIntent!=null&&Util.isFlag(mIntent.getFlags(), MainXposed.mPref.getInt(Common.KEY_FLOATING_FLAG, Common.FLAG_FLOATING_WINDOW)));
                    String packageName = Util.getFailsafeStringFromObject(previous, "packageName");
...
//						MovableWindow.DEBUG("startActivityLocked in flagtest [" + isMovable + "]");
//					}
                    String packageName = Util.getFailsafeStringFromObject(activityRecord, "packageName");
                    if(packageName==null)
